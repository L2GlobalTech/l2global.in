...
const SalesForceContainer: React.FC<SalesForceContainerProps> = ({ initialData }) => {
    const [serviceData, setServiceData] = useState<ServiceRecord | null>(initialData || null)

    useEffect(() => {
        let isMounted = true
        getPublicServiceBySlug('salesforce-consulting-implementation').then((data) => {
            if (isMounted && data) {
                setServiceData(data)
            }
        }).catch(console.error)
    ...
    // Map Dynamic Capabilities if present in Supabase
    ...
    return (
        <div>
            <ServiceHeroSection
                image={heroImg}
                sectionTitle={serviceData?.badge_text || 'Salesforce Solutions'}
                titleBefore={serviceData?.hero_title || 'Scalable & Intelligent'}
                titleAfter={serviceData?.hero_logo_text || 'Solutions'}
                linearText={serviceData?.hero_highlight || 'Salesforce'}
                description={serviceData?.hero_description || 'Empower your business with Salesforce-driven transformation. From CRM implementation to automation and AI-powered insights, we help you streamline operations, boost customer engagement, and drive measurable growth.'}
                tag1={serviceData?.hero_badges?.[0] || 'Certified Salesforce Experts'}
                tag2={serviceData?.hero_badges?.[1] || 'End-to-End CRM Solutions'}
            ...
            <ServiceAboutSection
                ...
                titleAfter={serviceData?.about_logo_text || 'Integration Matters'}
                linearText={serviceData?.about_highlight || 'Salesforce'}
